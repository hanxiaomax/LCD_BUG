...
//
// 1. Display clear                     1.显示清除
// 2. Function set: 					2.函数设置
//    DL = 1; 8-bit interface data 		  * DL=1 8位 interface 数据
...
//    I/D = 1; Increment by 1 			  *I/D=1 增加1 ？
//    S = 0; No shift 					  *S=0  ？
//
// Note, however, that resetting the Arduino doesn't reset the LCD, so we 
// can't assume that its in that state when a sketch starts (and the
// LiquidCrystal constructor is called).
//注意，复位arduino并不会复位LCD，因此我们并不能假设，当出现sketch时，Arduino就是在那个状态且液晶屏构造器被调用
...
void LiquidCrystalSerial::init(Pin strobe, Pin data, Pin clk)//LCD初始化
{
	_strobe_pin = strobe;//选通脉冲
	_data_pin = data;//数据	
	_clk_pin = clk;//时钟
	...
	bool fourbitmode = true;//4BIT模式开关
	if (fourbitmode)
		_displayfunction = LCD_4BITMODE | LCD_1LINE | LCD_5x8DOTS;//0x00|0x00|0x00=0x00 按位与，把有1的位置1
	else 
		_displayfunction = LCD_8BITMODE | LCD_1LINE | LCD_5x8DOTS;//0x00|0x00|0x00=0x01
